...
import firedrake
from firedrake import inner, grad, dx, min_value, max_value, Constant, derivative
...
############# geometry
data = xr.open_dataset('/g/data/rd53/wy2165/heard_island/codes/heard_island_obs.nc')
dem           = data['dem']
bathy_lagoon  = data['bathymetry_lagoon']
bathy_ocean   = data['bathymetry_sexton']
thk_millan    = data['icethickness_millan']
dem = dem - thk_millan + bathy_lagoon + bathy_ocean
b0 = icepack.interpolate(dem, S)
b = b0.copy(deepcopy=True)
h = icepack.interpolate(thk_millan, Q)
...
outdir = '/scratch/rd53/wy2165/heard_island/heard_island/'
with h5py.File(os.path.join(outdir, f'hs_hybrid_{mesh_disk}.h5'), 'w') as f:
    for i, thk in enumerate(hs):
        f.create_dataset(f'{i}', data=thk.dat.data[:])
        
with h5py.File(os.path.join(outdir, f'us_hybrid_{mesh_disk}.h5'), 'w') as f:
